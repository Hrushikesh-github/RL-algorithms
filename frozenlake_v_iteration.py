...
class Agent:
    ...
    # Calculate value of a action from a given state.
    def calc_action_values(self, state, action):                
        # action value, Q = sigma{prob * (r + (gamma*V(state)))}  Note that this is summed to all the states reached.

        # prob can be calculated as # of times the action is choosen / total # of actions
        target_counts = self.transition_table[(state, action)]
        total = sum(target_counts.values())
        action_value = 0.0
        
        # Loop over the target states, and add them up
    ...
    # Perform the value iteration algorithm (most of this is done in calc_action_values method), and update the value table
    def value_iteration(self):
        for state in range(self.env.observation_space.n):
            state_values = [self.calc_action_values(state, action) for action in range(self.env.action_space.n)]
